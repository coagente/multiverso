...
internal/schedule/trace.go        +finish_ms, +committed, +score_basis on Considered;
                                  +commit_basis, +commit_set, +scarce,
                                  +uncommitted_ms on Step (FOUR, not three — see §8);
                                  +adaptive_rule on Constants. Observational, additive.
cmd/mvo/race.go                   --selector={voc2|voc}. AMENDED: `voc` REMAINS the
                                  default for --schedule=adaptive; the revision ships
                                  behind the flag (§7.6, falsifier F-10)
...
scripts/eval.sh                   AMENDED: a global --selector, so the adaptive cell
                                  races ONE rule per protocol run and the before/after
                                  is TWO runs with a doubled query count (§7.4)
```
...
>
> **AMENDED IN IMPLEMENTATION — the commit order's score holds `discount_bp` and `executor_bp` at their maximum, and the amendment is the price of the sentence above.** Read literally, the commit order ranks WORLDS by the score of the ONE RUNG each is next entitled to buy. Those two terms price a *rung's* marginal evidence value and are not comparable across worlds standing at different ladder positions: on the shipped default's own ladder `tree-guard` is control-plane (`executor_bp` 10 000) and both pytest kinds are candidate-process (5 000), so a world that has just bought its guard offers a rung worth **half** what an untouched world's guard is worth, whatever either world's finish cost. Measured on §1.1's own arithmetic: the literal reading commits to world A at step 1, to world B at step 2 — B's untouched guard outscores A's collect on the executor weight alone — and back again, buying **5 receipts for 1 275 ms** and finishing a world only by accident of where the churn stopped. **A commitment that moves every step is not a commitment**, and it destroys exactly the property [decision 2](#decision-2) says the denominator produces. Held at maximum the order reads *worlds that can still change the decision first, then cheapest to FINISH, then control-plane order* — decision 3's stated objective unchanged — and the same fixture then buys **3 receipts for 982 ms and finishes a world**, which is [§7](#7-pre-registration)'s P1 and P2 to the millisecond. `value_bp` itself is untouched: the discount and the executor weight still order the queue, still appear on every row, and still are what M2d correlates.
...
>
> **AMENDED IN IMPLEMENTATION — the override lapses only WHILE THE POOL IS COMMITTED TO FINISHING SOMEBODY ELSE, and the amendment is this decision's own justification read at its strength.** The argument above — *"buying its prefix cannot put it in the pass set … it is not a choice to withhold, it is the absence of an alternative"* — is about the **pass set**, and it is sound there. It misses the **escalation layer**, and the miss is not hypothetical. At a budget below **one** ladder no world is completable, so on the very first step every hard gate on every world lapses at once, nothing is admissible, and **the race buys nothing at all**. `Decide` then reports `on_all_worlds_failed_machinery` — *"no world produced conclusive evidence"* — which files unbought evidence under **broken machinery**, the exact misattribution [M2b §3.4](M2b-adaptive-scheduler.md#34-the-starved-stop-and-the-escalation-rule-m2a-owes-but-does-not-have) exists to remove, and it makes [`on_evidence_incomplete`](M2b-adaptive-scheduler.md#decision-14) unobservable: that rule needs one world that passed everything it *paid for*, and nothing was paid for. Measured: acceptance step 5e, whose fixture declares the rule and whose control does not, lost **both** halves — the rule's 4-of-4 ESCALATE and the control's REJECT — so decision 14's effect stopped being attributable at all. The flip test does not catch it, because while any world lacks its first receipt the machinery rule dominates every bracket outcome, so `fail-closed` moves nothing and `flip` is honestly 0; this is the same "buys nothing at all" bootstrap [M2b decision 3](M2b-adaptive-scheduler.md#decision-3)'s amendment already refused by name. **With `C` empty there is no somebody else**, no reservation is being protected, the money has no better use, and [M2b decision 3c](M2b-adaptive-scheduler.md#decision-3c) stands verbatim — which is the same regime [decision 3](#decision-3) already calls "M2b decision 8 exactly". The price is paid where the doc predicted the opposite: after the head world completes and the remainder cannot finish the rival, `C` is empty, so the rival's affordable prefix **is** bought and P2's *"≈ 982 ms, 3 receipts, ≈ 547 ms unspent"* becomes ≈ 1 275 ms and 5 receipts — [§7.5](#75-the-null-that-would-be-honest)'s convergence on the ladder, arriving one block earlier than expected, and [P3](#71-the-mechanical-predictions-checkable-from-the-trace-alone-and-needing-no-labels)'s behaviour for every policy rather than only for the one that declares the rule.
...
<a id="7-0-post-hoc"></a>
> **POST-HOC NOTICE — READ BEFORE §7.1. P1 AND P2 WERE NOT CONFIRMED OUT OF SAMPLE. THE RULE WAS AMENDED TWICE AFTER MEASUREMENT AND THE AMENDMENTS ARE WHAT MAKE THEM COME OUT.**
>
> §7 as committed is genuine — it was written before any measurement, and it is unchanged below except for this notice and for [F-10](#73-falsifiers--results-that-mean-the-fix-did-not-work). What moved is the **rule**, twice, and both amendments are written from measurement and say so:
>
> 1. [Decision 3's amendment](#decision-3) holds `discount_bp` and `executor_bp` at their maximum in the commit-order score. Its own text records the before and after: the literal reading *"buys **5 receipts for 1 275 ms** and finishes a world only by accident"*, and after the amendment *"the same fixture then buys **3 receipts for 982 ms** and finishes a world, which is §7's P1 and P2 to the millisecond."* **That sentence is the tell.** The amendment was chosen with P1 and P2 in view.
> 2. [Decision 5's amendment](#decision-5) lapses the hard-gate override only while the pool is committed to finishing somebody else — and separately records that **P2 as written was falsified**: *"P2's 982 ms, 3 receipts, 547 ms unspent becomes 1 275 ms and 5 receipts."*
>
> So the honest reading of the order of events is: predictions written → rule implemented → rule measured → rule amended (twice) → predictions re-checked. **P1 and P2 are POST-HOC.** They are a description of a rule that was tuned until they held, not an out-of-sample confirmation, and no reader may treat them as the latter. Both amendments are defensible on their own arguments — a commitment that moves every step is not a commitment; a budget below one ladder must not make the race buy nothing at all — and neither argument needs P1 or P2 to stand.
>
> **P2's own falsification is recorded twice and its consequence once: the arm converges on the ladder, one block earlier than §7.5 expected.**
...
| **F-9** | The **freeze check does not refuse** on an unmodified `eval/freeze/local-derived-v1.json` **when the binary's default rule has moved** | [decision 8](#decision-8) did not land, and the instrument still cannot see rule changes. *Amended after the fact in exactly one respect, and the amendment is stated rather than smuggled: the revision shipped as a selectable arm and NOT as the default, so nothing moved and the live check correctly does not refuse. The refusal is exercised against a rules map naming `voc2`, and a second test asserts the committed freeze accepts the shipped binary — because a freeze that is refused on every run trains every reader to pass `--unfreeze`, which the file's own notes call worse than not checking.* |
| **F-10** | **ADDED AFTER MEASUREMENT, AND IT FIRED. It is the falsifier this pre-registration was missing: every falsifier above is stated against the LADDER or against the published metric, and none is stated against `voc` — the arm being replaced and the shipped default.** *If `voc2` admits (SELECT) at a budget where `voc` rejects and the full-evidence decision is ESCALATE, the fix traded a false rejection for a false admission.* | **OBSERVED.** `testdata/toyrepo/patches-tie`, shipped default policy, `--budget-basis=predicted`, warm cost table. Unbounded reference: **ESCALATE**, `on_ranking_tie`, S = 1 998 ms. At B = 1 200 / 1 400 / 1 600 / 1 800 / 2 000: **`voc` REJECT 5/5** (596–634 ms, 4 receipts, 2+2) and **`voc2` SELECT 5/5** (1 006–1 360 ms, 4–5 receipts, 3+1 or 3+2) with the rival's suite never bought. At B = 2 100 ≈ S all three arms ESCALATE, so the divergence is exactly the budgeted band the block exists for. Through the project's own harness: `schedule-compare.sh --patches patches-tie --budget 1400 --replicates 4 --arm-a --selector=voc --arm-b --selector=voc2` → `REJECT/SELECT` 4/4, both arms deterministic given the rotation. **F-4 could not have caught it**: F-4 is stated against the ladder and `voc2` *ties* the ladder here (finding: identical decision, spend within 10 ms, identical per-world receipt counts at 6/6 budgets). The consequence is [§7.6](#76-what-shipped-and-what-did-not) |
...
- `scripts/eval.sh` on `local-derived@v1`, shipped default policy, **both `on_evidence_incomplete` configurations** ([F14](M2b1-budgeted-fixed-arm.md#f14)), R = 3, reference replicated 3× with B from the median `minspend`, arms `{A1 ladder, A2a voc, A2b voc2}` plus the derived arms unchanged. This produces P9–P16. **Cells: 3 budget levels × 2 policy configurations. No others, and no cell is re-scored.**
  ...
  > **AMENDED IN IMPLEMENTATION — THERE IS NO `A2b` ARM, AND THIS IS A TWO-RUN PROTOCOL WITH A DOUBLED QUERY COUNT.** §7.4 as written declares three arms in one run; [§5.3](#53-what-is-forbidden-in-this-block-in-writing) forbids touching `internal/eval/arms.go`, which is where an arm would have to be added. The doc contradicts itself and the implementation resolved it silently. What shipped is a single global `mvo-eval run --selector=…`, applied to `eval.ArmAdaptive` alone, so producing P9–P16 needs **two full protocol runs** — which **scores every declared cell twice**: 12 leaderboard queries against the 6 declared, and [§5.2](#52-the-unfreeze-contract-executed-rather-than-described) says a second scoring of a cell is a second query that must appear in the published count. It also **loses the within-run interleaving and rotation pairing** that makes a per-instance `voc`-vs-`voc2` difference attributable; only the unlabelled half (`schedule-compare.sh --arm-a --selector=voc --arm-b --selector=voc2`) pairs correctly. The amendment is recorded here rather than fixed, because adding the arm is the change §5.3 forbids and this block does not get to grant itself an exception to its own instrument rules.
  >
  > **AND THE LABELLED HALF CARRIES NO INFORMATION ABOUT THE REVISION AT ALL, MEASURED.** `mvo-eval` builds a **fresh workspace per instance**, so no oracle kind has a local fit, `finish_ms` is UNKNOWN, and [decision 1](#decision-1)'s fallback fires on every step: `commit_basis: "unpriced-fallback(pytest-collect,pytest-suite,tree-guard)"`, `scarce: false`, `|C| = 0`. `voc2` runs M2b's rule for the whole race, so **every cell is identical between the two rules** — same TCAR, same FAR, same `FRR_reachable`, same ESC, per instance, in all six cells and both policy configurations. **P9–P16 are unevaluable as the harness stands**, and the honest fix — a per-instance warm-up race, or `--budget-basis=predicted` — changes the affordability of *every* arm and is therefore a change to the instrument, which §5.3 forbids in this block. It is the first thing the next block must build.
...
> **MEASURED, AND THE COMMITMENT IS HONOURED: `voc2` IS THE LADDER UNDER SCARCITY, MECHANICALLY AND NOT COINCIDENTALLY.** `Table.Predict` is keyed on `(kind, units)` and holds nothing per world, so on symmetric worlds `finish_ms` is **identical** for every world at the same ladder position (measured: 1 034 ms for both worlds on the tie fixture at B = 1 500). With `flip` equal too, `newCommitPlan`'s sort falls through score and through finish to its terminal key — `st.OrderIndex`, the control-plane world order — **which is `selectorLadder`'s entire ranking rule.** Measured consequences: identical decision, spend and per-world receipt counts against the ladder at 6/6 budgets on `patches-tie`; identical at 3/3 budgets on the `23-schedule_starvation` duel with a warm table; 0 % disagreement against the ladder at all three levels with spends within 10 ms.
>
> So **the claim that the arm is adaptive under scarcity is withdrawn**: under the only regime the revision exists for, the value model (`flip × discount_bp × executor_bp`) decides nothing about who gets the money, and the commit order is the candidate ordinal. That is this section's own predicted null, arriving as predicted, and by the commitment above **the next block is a per-world cost signal or an asymmetric corpus, not another rule.**

<a id="76-what-shipped-and-what-did-not"></a>
### 7.6 What shipped, and what did not

**`voc2` ships as a selectable arm. It does NOT ship as the default. `AdaptiveRuleDefault` is `voc`.**

Two measurements decided it, and neither was reachable from §7 as written:

1. **[F-10](#73-falsifiers--results-that-mean-the-fix-did-not-work) fired.** Under the shipped default policy, across the whole informative budget band, the revision converts a full-evidence ESCALATE into a SELECT where the rule it replaces rejects. §3.3 pre-registered the *direction* of that trade and argued it was not a new hole; what it did not do is state a falsifier against `voc`, so the trade was measured only after the rule had been written, amended twice and made the default. A rule may make that trade — the ladder has made it since M2b.1 — but it may not become the **shipped default** in the same block that first measures it, on a corpus that cannot price it.
2. **No gate measures the revision at all.** The adversarial corpus builds a fresh workspace per vector (`mkrepo`), so no kind is priced and `voc2` falls back to M2b's rule on every step; 21 of the 22 vectors carry no budget, and the 22nd (`24-schedule_budget_burn`, B = 400) reproduces `unpriced-fallback`, `scarce: false`, `|C| = 0` and a byte-identical purchase order to `voc`. The eval cells are identical between the rules for the same reason (§7.4's amendment). **`adversarial 22/22` and `FAR unchanged` are true statements about `voc`** and carry no information about the revision. §6's claim that the corpus is the block's blocking gate is therefore **vacuous for the rule this block adds**, and saying so is the point of writing it down.

**What promoting it needs, stated so the next block cannot skip it:** a cell in which `commit_basis` is `reserved` on at least one step of a **labelled** instance (a per-instance warm-up race, or `--budget-basis=predicted`, in `mvo-eval`); adversarial vectors that attack allocation carrying budgets **and** a warm table, with the baseline re-recorded as its own declared change; and either a shipped default policy that declares `on_evidence_incomplete`, or a reservation that refuses to commit while the uncommitted remainder cannot buy the rivals' decision-relevant hard gates. `scripts/accept.sh` step **m2b2-8e** is the gate that holds the line meanwhile: on `patches-tie` under the shipped default at two budgets in the informative band, the default adaptive arm must not turn the full-evidence ESCALATE into a SELECT. It passes under `voc` and fails under `voc2`, and it prints both.
...
// schedule.Step — FOUR additive observational fields. (AMENDED IN
// IMPLEMENTATION: this block and the module layout both said THREE and both
// omitted commit_basis, which decision 1 in fact REQUIRES — "recorded as
// commit_basis: 'unpriced-fallback' with the kinds named" — and which is the
// field a reader needs to tell WHICH RULE allocated a step. The trace-record
// test's own comment already said "four on a step".)
type Step struct {
    // … unchanged …
    CommitBasis   string   `json:"commit_basis"`   // "" | not-scarce | reserved | unpriced-fallback(kinds)
...
**`scripts/accept.sh`**, five steps (`m2b2-8a…8e`); none changes the shipped default. **AMENDED IN IMPLEMENTATION**, and the accounting is: 8a–8d shipped as written; **8e as declared below** (the freeze) is a unit test in `internal/eval/instance_test.go` — it needs no corpus and asserting it in the script would need one; **8f** (AG-7 under live race) folded into the existing step 5g assertions, whose forbidden list gained `commit_set`, `commit_basis`, `finish_ms`, `uncommitted_ms`, `score_basis`, `adaptive_rule`, `unpriced-fallback` and `voc2`; and **the shipped 8e is a different and harder step** — the decision-quality gate [§7.6](#76-what-shipped-and-what-did-not) demanded, which none of 8a–8d could fail:
...
- **8e (as declared) — the freeze refuses.** *Shipped as `internal/eval` unit tests:* `TestFreezeRefusesOnAMovedAllocationRule` names `adaptive_rule: "voc" -> "voc2"` on a moved rule, and `TestAFreezeWrittenByThisBuildIsAcceptedByThisBuild` pins the fixed point the check did not have — `FreezeFile.Rules` is `json:"-"` and there was no `MarshalJSON`, so `mvo-eval freeze` emitted a file with no `adaptive_rule` key that the next read refused forever.
- **8e (as shipped) — THE GATE THAT CAN REFUTE THE BLOCK.** On `testdata/toyrepo/patches-tie` under the **shipped default policy**, where the unbounded reference decides ESCALATE on `on_ranking_tie`, at two budgets in the informative band (60 % and 80 % of the reference spend): the **default** adaptive arm's decision is **not SELECT**. A SELECT there is a starved admission with the rival's hard gates never bought — [vector 25](M2b-adaptive-scheduler.md#new-adversarial-vectors) — and every other step in this list asserts the direction the block wants. The step prints the `--selector=voc2` decision beside it, so the trade is in the acceptance log and not only in a design document.
- **8f — AG-7 under live race.** *Folded into step 5g,* whose live-race forbidden list over recorded agent context and transcript blobs gained every `schedule.*` key this block adds.

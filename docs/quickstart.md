...
`--budget-oracle-ms` is the scheduler's pool: the total oracle milliseconds a race may spend, summed across worlds. **`0` means unbounded, which is the M1 ladder** — every rung of every world that is still alive gets bought — so an intent recorded before M2b races exactly as it always did. Give it a positive value and the scheduler starts declining purchases, and `mvo explain --schedule` shows which and why.

**The budget binds under two arms, and `--schedule` picks which** (M2b1). `--schedule=adaptive` is the M2b value-of-computation rule: buy the purchase most likely to move the decision per predicted millisecond. `--schedule=fixed-budget` is the depth-first ladder **given the same money**: worlds in the control plane's own order (candidate ordinal, never world digest), each world's rungs in policy gate order, stopping when the pool is empty. Both charge one pool through one affordability predicate and emit the same `schedule.*` trace, which is what makes them comparable field by field — `scripts/schedule-compare.sh` races them paired and asserts it. `--schedule=fixed` is still there and is **not** an arm of that comparison: it is the UNBUDGETED M1 ladder, it reads `max_oracle_ms` never, and it records no trace. The traced reference is `--schedule=fixed-budget` with `--budget-oracle-ms 0`.

...
| A candidate that **hands the budget on** by failing its own cheapest gate — the commit handoff | **OPEN by construction, not exercisable today** | Under M2b.2's reservation the pool is committed to a prefix of the control-plane world order, so a candidate that eliminates itself releases its whole reserve and the next world in the order gets it. It is a *conditioning* channel and not a steering one: the order is control-plane and rotated, so the attacker cannot choose who benefits, only that somebody does — and self-elimination is terminal for the attacker. It ships as a named vector with no fixture, because building it needs a colluding pair and an agent. It strengthens a prerequisite that already exists twice: the generator prompt's `"candidate k of N"` must go before a budgeted arm faces untrusted generators. See [M2b.2 §6](design/M2b2-finishing-rule.md#6-the-scheduler-as-attack-surface-what-the-revision-adds) |
| A race that **admits while a rival's gate went unbought** — the starved admission | **OPEN by poverty, CLOSED by choice** | Withholding a hard-gate receipt drops a world out of the pass set, and `SELECT` is *not* monotone in the pass set: the tie or shortfall that would have escalated never materialises, and the race admits where the exhaustive ladder asked a human. A red team measured 8 of 8 admissions against 3 of 3 escalations at the same budget. The scheduler can no longer *choose* it — a hard gate on a live world is bought whatever the value model says — but a budget that genuinely runs out still leaves it. A policy declaring `on_evidence_incomplete` escalates instead (measured 4 of 4); the shipped default does not declare it until M2d measures its base rate. Every SELECT rationale now names the rivals it never measured, in every policy. **M2b.2 makes it more reachable rather than less, MEASURED IT, AND DID NOT SHIP THE RULE AS THE DEFAULT BECAUSE OF WHAT IT MEASURED**: under the finishing rule a starved adaptive race finishes somebody, so `passCount` can be 1 with rivals unmeasured, where the old rule finished nobody and rejected. An earlier draft of this row called that "a false admission the same corpus has never produced". **The corpus produces one, in a single harness invocation on a committed fixture.** On `testdata/toyrepo/patches-tie` under this same shipped default, where the unbounded reference ESCALATEs on `on_ranking_tie`, `--selector=voc2` SELECTs at every budget from 1 200 to 2 000 ms while `--selector=voc` REJECTs at every one of them, with the rival's suite never bought; at the reference spend all three arms ESCALATE again. So the trade is real, it is one-way at the shipped default, and `--schedule=adaptive` still allocates by `voc`. It is the same starved-race semantics the budgeted ladder has had since M2b.1 — which `--schedule=fixed-budget` makes available to anyone who wants it, with this row as the caption |
